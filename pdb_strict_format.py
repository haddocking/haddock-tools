...
aa = ["ACE", "CTN", "ALA", "CFE", "CYS", "CYM", "CYF", "CSP", "ASP", "GLU", "PHE", "GLY", "HIS", "NEP", "ILE", "LYS",
      "ALY", "MLY", "MLZ", "M3L", "LEU", "MET", "ASN", "PRO", "GLN", "ARG", "SER", "SEP", "THR", "THP", "TOP", "VAL",
      "TRP", "TYR", "PTR", "TYP", "TYS", "TIP", "HYP", "HEB", "HEC", "WAT", "PNS"]
bases = ["ADE", "CYT", "DOC", "GUA", "DGP", "URI", "THY", "THJ", " DG", " DC", " DT", " DA", "DG ", "DC ", "DT ", "DA ",
         "  A", "  G", "  C", "  T", "  U"]
aa += bases
weirdbases = [" A ", " G ", " C ", " T ", " U "]  # unusual one-letter bases
aa += weirdbases
knownligands = ["DFO", "ADY", "ACD", "ACT", "ACN", "BDY", "BEN", "CHE", "DME", "ETA", "EOL", "BUT", "THS", "AMN",
                "PHN", "URE", "IMI", "MER"]
aa += knownligands
...
class PDBParser(object):
    ...
    def _parse_atom_line(self, line, chain_id_check):
        ...
        if record.strip() == "ATOM" and resn not in aa:
            raise PDBParsingError('Residue name ("{}") is unknown, please check the syntax or replace ATOM by HETATM'.
                                  format(resn), line)
        ...
        try:
            ...
            raise PDBParsingError('X,Y,Z coordinates, occupancy, and temperature (b) factors must be decimal numbers.',
                                  line)
        if chain_id_check and chain == " ":
            if not segid:
                ...
                raise PDBParsingError('No chain_id found but segid detected,\n'
                                      'use http://github.com/haddocking/pdb-tools/pdb_segxchain.py to exchange '
                                      'chain_id for segid', line)
            ...
            for iline, line in enumerate(pdb_handle):
                ...
                if record in valid_records:
                    if line.startswith('MODEL'):
                        if model_open:
                            raise PDBParsingError('It seems an ENDMDL statement is missing at line {0}'.format(iline),
                                                  line, True)
                        model_open = True
                        ...
                        try:
                            ...
                            raise PDBParsingError('Could not parse the PDB file at line {0}\n{1}'.
                                                  format(iline + 1, error), line, True)
                        else:
                            # Register atoms
                            ...
                            # Register residue
                            _, atname, _, _, chain, resi, icode, _, _, _, _, _ = atom
                    ...
                    raise PDBParsingError('Could not parse the PDB file at line {0}: record unknown ({1})'.
                                          format(iline, record), line, True)
        if len_warning:
            print("WARNING: At least one ATOM/HETATM line consists of less than 80 columns. \nTo follow the official "
                  "wwPDB guidelines, please use http://github.com/haddocking/pdb-tools/pdb_linewidth to format your "
                  "PDB file.\nOfficial PDB format guidelines can be found here: "
